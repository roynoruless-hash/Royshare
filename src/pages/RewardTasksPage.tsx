...
export default function RewardTasksPage({ userIdProp, taskIdProp, onBack }: { userIdProp?: string; taskIdProp?: string; onBack?: () => void } = {}) {
  ...
  const [monetagDebug, setMonetagDebug] = useState<any>(null);
  ...
  useEffect(() => {
    ...
    if (adWatchedSuccessfully && !isCompletedSuccess && !isVerified && !submitting) {
      const tg = (window as any).Telegram?.WebApp;
      const realUserId = tg?.initDataUnsafe?.user?.id ? String(tg.initDataUnsafe.user.id) : userId;
  ...
  const handleWatchMonetagAd = async () => {
    ...
    try {
      ...
      await (window as any).show_11210088(adOptions).then((result: any) => {
        ...
        console.error("[MONETAG_FLOW] SDK Error caught:", error);
        setMonetagDebug((prev: any) => ({
          ...prev,
        ...
        if (isExplicitSkipOrClose) {
          ...
          setMonetagError(error?.message || "Failed to launch advertisement.");
        }
      });
    } catch (err: any) {
      console.error("[MONETAG_FLOW] Sync execution failed:", err);
  ...
  const handleClaimMonetagReward = async () => {
    ...
    const tg = (window as any).Telegram?.WebApp;
    const realTelegramId = tg?.initDataUnsafe?.user?.id ? String(tg.initDataUnsafe.user.id) : userId;
  ...
  if (showRewardAlreadyClaimed) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#0b0f19]/95 backdrop-blur-sm text-white p-6 text-center z-50">
        <motion.div 
